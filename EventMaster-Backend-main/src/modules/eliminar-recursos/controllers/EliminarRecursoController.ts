import express, { Request, Response, Router } from "express";
import { DependencyContainer } from "../../../shared/utils/DependencyContainer";

// importacion de middlewares
import { authMiddleware } from "../../../shared/middlewares/authMiddleware";

export class EliminarRecursoController {
  private router: Router;
  private path: string = "/api/eliminar-recursos";

  // Use Cases (inyectados desde el contenedor)
  private eliminarRecursoUseCase = DependencyContainer.getEliminarRecursoUseCase();

  // Middleware
  private verifyOrganizerInEvent = DependencyContainer.getVerifyOrganizerInEvent();

  constructor() {
    this.router = express.Router();
    
    // aplicar middleware a todas las rutas
    this.router.use(authMiddleware);

    // inicializar rutas
    this.initializeRoutes();
  }

  private initializeRoutes(): void {
    // 1. Endpoint para eliminar recurso de evento
    this.router.delete("/", this.verifyOrganizerInEvent, this.eliminarRecurso.bind(this));
  }

  // Handler: Eliminar recurso
  private async eliminarRecurso(req: Request, res: Response): Promise<void> {
    try {
      const { evento_id, recurso_id } = req.body;
      const usuarioId = (req as any).user?.id;

      if (!usuarioId) {
        res.status(401).json({
          success: false,
          message: "Usuario no autenticado"
        });
        return;
      }

      const result = await this.eliminarRecursoUseCase.execute({
        evento_id,
        recurso_id
      }, usuarioId);

      res.status(200).json(result);
    } catch (error: any) {
      console.error("Error eliminando recurso:", error);

      if (error.message.includes('requerido')) {
        res.status(400).json({
          success: false,
          message: error.message
        });
        return;
      }

      if (error.message.includes('no encontrado')) {
        res.status(404).json({
          success: false,
          message: error.message
        });
        return;
      }

      if (error.message.includes('no está compartido') || 
          error.message.includes('No tienes permisos')) {
        res.status(403).json({
          success: false,
          message: error.message
        });
        return;
      }

      res.status(500).json({
        success: false,
        message: "Internal server error"
      });
    }
  }

  // Método público para obtener el router configurado
  public getRouter(): Router {
    return this.router;
  }

  public getPath(): string {
    return this.path;
  }
}
